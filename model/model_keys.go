...
func (m Model) handleRightPaneKey(key string) (tea.Model, tea.Cmd) {
	...
	if handler := rightPaneModeHandler(m.focusedMode()); handler != nil {
		if next, cmd, handled := handler(m, key); handled {
			return next, cmd
		}
	}
	return m.handleRightPaneSharedKey(key)
}
...
func flowManualMergeEligible(record flowstore.FlowRecord) bool {
	...
	merge, ok := flowstore.FindPhaseByKind(record, string(flowstore.KindMerge))
