...
func TestFlowPlanPromptAppendsPhaseDoneInstruction(t *testing.T) {
	...
	want := strings.Join([]string{
		...
		"Create and persist the plan with $APPROACH_BIN plan save, link it back with $APPROACH_BIN flow plan set, then report Flow persistence failures explicitly before ending.",
		"If the task references a GitHub issue, link it with $APPROACH_BIN flow issue set using the issue number and URL; when only #N is given, derive the URL from an unambiguous GitHub origin remote or note the ambiguity instead of guessing.",
		"",
...
func TestFlowGenericPhasePromptPreservesContextAndAppendsPhaseDoneInstruction(t *testing.T) {
	...
	want := appendFlowDoneInstructionForTest(strings.Join([]string{
		...
		"",
		"Advance this phase with `$APPROACH_BIN flow phase set` only after the corresponding work is complete, blocked, or needs attention.",
	}, "\n"))
