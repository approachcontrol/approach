...
func TestModel_RLaunchesUntrackedEmbeddedRepairFromBothFlowSurfaces(t *testing.T) {
	for _, active := range []bool{false, true} {
		t.Run(fmt.Sprintf("active=%v", active), func(t *testing.T) {
			...
			for _, want := range []string{record.FlowID, "blocked", "persisted metadata is inconsistent", "$APPROACH_BIN flow read", "phase reset", "phase restart", "Do not launch the next phase"} {
...
func TestModel_GLaunchesFlowPhasePRCreationWithMinimalPrompt(t *testing.T) {
	...
	wantPrompt := appendFlowDoneInstructionForTest(strings.Join([]string{
		...
		"After the PR exists, run `$APPROACH_BIN flow pr set --flow-id flow-1 --provider github --number <number> --url <url> --head flow/pr --base <base>` before completing this phase.",
		"",
...
func TestModel_GLaunchesFlowPhaseMergeWithStructuredReportingPrompt(t *testing.T) {
	...
	for _, want := range []string{
		...
		"$approach_bin flow merge set --flow-id flow-1 --status merged",
		"--commit <merge-commit>",
		"--merged-at <rfc3339>",
		"$approach_bin flow phase set --flow-id flow-1 --phase-id merge --status completed",
		"blocked",
	} {
		if !strings.Contains(prompt, want) {
...
func TestModel_NewFlowPlanNowRoutesFormThroughProductionLifecycle(t *testing.T) {
	...
	for _, want := range []string{"approach-flow", "write\nthe plan", "$approach_bin plan save", "$approach_bin flow plan set"} {
		if !strings.Contains(prompt, want) {
