...
func TestFlowLaunchLifecycleBoundary(t *testing.T) {
	...
	sinks := map[string]bool{
		...
		"runFlowLifecycleTmuxLaunchWithStatus":      true,
	...
	expectedSinkReferences := map[string]map[flowLaunchFunctionKey]bool{
		...
		"runFlowLifecycleTmuxLaunchWithStatus": {modelFlowLaunchFunction("handoffFlowLaunchTmux"): true},
		// handleSliceSelectedEpic is Ready-Bead `S`: an explicitly non-Flow
		// route whose context carries no Flow marker, so
		// flowLaunchContextRequiresLifecycle refuses it if one is ever added.
		"launchAgentForBackend":                     {modelFlowLaunchFunction("Update"): true, modelFlowLaunchFunction("routeNonFlowSavedSessionResume"): true, modelFlowLaunchFunction("launchAgentAtPath"): true, modelFlowLaunchFunction("launchAgentAtPathWithBranch"): true, modelFlowLaunchFunction("handleSliceSelectedEpic"): true},
