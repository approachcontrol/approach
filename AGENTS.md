...
- `embeddedterm/` — runtime-only PTY process management for embedded sessions-mode CLI resumes. It owns PTY start, output capture, live visible lines, input forwarding, resize, lifecycle state, and termination behind a small API so model tests can fake terminals.
...
- `ui/` — stateless lipgloss rendering from a `RenderParams` snapshot, including Flow recovery labels (`recover-worktree`, `await-session`, `session-mismatch`, `missing-session-id`, `missing-pr`) and sessions-mode embedded terminal headers/live lines.
...
- Embedded sessions-mode terminals are runtime-only and not persisted or restored. Keep PTY mechanics in `embeddedterm`; model should own slot selection, `ctrl+g` prefix routing, picker/confirm state, and conversion from saved sessions to `actions.AgentCommand` contexts. `codex-app` session resumes continue to use the external deep-link path.
