...
- `actions/` — git mutations (worktree create/remove/prune/unlock, branch delete, stash drop, fetch `--prune`, pull `--ff-only`), clipboard, VS Code, tmux/Zellij/terminal launches, and Codex/Claude launch/resume command construction with wtui hook metadata.
...
- `WORKTREE_ROOT` overrides `[scan].root`; `TERMINAL` overrides `[terminal].command` for launches outside tmux/Zellij. Editor/provider/launch config fields are parsed foundation only. All store roots must be absolute.
- Sessions, plans, and Flows share one artifact root (default `$XDG_STATE_HOME/wtui/sessions/v1` or `~/.local/state/wtui/sessions/v1`); moving or cleaning it moves saved artifacts too. Root precedence: `--state-root` > `WTUI_FLOW_STATE_ROOT` (Flows only) > `WTUI_PLAN_STATE_ROOT` > `WTUI_SESSION_STATE_ROOT` > `[sessions].root` > default. wtui-launched agents get the resolved root exported.
