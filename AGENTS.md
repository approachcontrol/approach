...
- `claudestream/` — renders the `claude --print --output-format stream-json` JSONL event stream into readable terminal lines (init header, thinking/tool-call/tool-result/text blocks, closing summary). It is a stateful, streaming `embeddedterm.OutputTransform`; non-JSON lines pass through verbatim so provider errors stay visible.
