...
directly, while `claude --print` is run with `--output-format stream-json` and wtui
translates those events into readable lines (thinking, tool calls and results, the
final answer, and a closing summary) so a Claude phase shows live progress as it
works instead of a blank terminal. While a Flow terminal is open,
